<?php
	...
	if(strlen($goods_id) <= 0 || !is_numeric($goods_id)){
		die("<script>location.href='goods_new_delete.php'; alert('제거할 상품 ID를 선택해 주세요.');</script>");
	}
	...
	if(!mysqli_num_rows($res)){
		die("<script>location.href='goods_new_delete.php'; alert('ID : {$goods_id} 신규 상품은 등록되어 있지 않습니다.');</script>");
	}

	$sql = "DELETE FROM index_goods_view WHERE view_name = 'new_goods' AND goods_id={$goods_id}";
	$connect->query($sql) or die();
	die("<script>location.href='goods_new_delete.php'; alert('ID : {$goods_id} 신규 상품을 제거 했습니다.');</script>");
?>