<?php
	...
	if(strlen($search)===0){
		die("<script>location.href='goods.php'; alert('검색할 상품 제목을 입력해 주세요.');</script>");
	}
	
	if(strlen($search)<2){
		die("<script>location.href='goods.php'; alert('검색할 상품 제목을 2자 이상으로 입력해 주세요.');</script>");
	}

	$count_sql = "select deleted_or_not from goods where deleted_or_not = 0 and title like '%{$search}%'";
	...
	if(!mysqli_num_rows($count_res)){
		die("<script>location.href='goods.php'; alert('검색된 상품이 없습니다.');</script>");
	}
	...
	$sql = "select id, title, member_price, non_member_price, product_condition from goods where deleted_or_not = 0 and title like '%{$search}%' ORDER BY id DESC LIMIT {$min_count},{$number_of_data}";
				...
				<div class="goods_list">
					...
					<div class="paga_view">
						<?php
							...
							if(1 < $page){
						?>
							<a class="arrow" href="?search=<?php echo $search; ?>&page=1">《</a>
							<a class="arrow" href="?search=<?php echo $search; ?>&page=<?php echo $page-1; ?>">〈</a>
						<?php
							}
							for ($i=$for_min; $i <= $for_max; $i++) { 
						?>
						<a <?php if($page == $i){ echo "class='page_check'"; } ?> href="?search=<?php echo $search; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
						<?php
							}
						?>
						<?php
						...
						?>
						<a class="arrow" href="?search=<?php echo $search; ?>&page=<?php echo $page+1; ?>">〉</a>
						<a class="arrow" href="?search=<?php echo $search; ?>&page=<?php echo $page_count; ?>">》</a>
						<?php
							}
						?>
					</div>
