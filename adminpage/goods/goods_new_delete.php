...
<body>
	<div class="parents_div">
		...
		<div class="right_bar">
			...
			<div class="main_view">
				<h1>신규 상품 제거(index)</h1>
				<div class="good_view">
					<table>
						<tr>
							<td>상품 ID</td>
							<td>
								<form action="goods_new_delete_check.php" method="post" name="goods_view_check">
									<select name="goods_id">
										<option value="none">상품 ID</option>
										<?php
											$new_sql = "select goods_id from index_goods_view where view_name = 'new_goods'";
											$new_res = $connect->query($new_sql) or die();
											while($data = mysqli_fetch_array($new_res)){
										?>
										<option value="<?php echo $data["goods_id"]; ?>"><?php echo $data["goods_id"]; ?></option>
										<?php
											}
										?>
									</select>
						...
						<tr>
							<td colspan="2">
								<button onclick="goods_new_delete()" type="button">제거</button>
							</td>
						</tr>
					</table>
				</div>
